...
fn main() {
    let mut scarvie_colors = load_scarvie_colors_ordered();
    assert!(!scarvie_colors.is_empty());

    let window = three_d::Window::new(three_d::WindowSettings {
        title: "Aftiktuna: Scarvie Color Editor".to_string(),
        min_size: SIZE,
        max_size: Some(SIZE),
        ..Default::default()
    })
    ...
    window.render_loop(move |mut frame_input| {
        ...
        let (_, entry) = scarvie_colors.get_index(selected_index).unwrap();
        draw_examples(entry.color_data, &scarvie_model, &frame_input);
...
fn draw_examples(
    ...
    scarvie_model: &Model<three_d::Texture2DRef>,
