...
type ColorMap = IndexMap<SpeciesColorId, SpeciesColorEntry>;

fn main() {
    let mut colors = load_colors_ordered();
    assert!(!colors.is_empty());

    let window = three_d::Window::new(three_d::WindowSettings {
        title: "Aftiktuna: Pagepoh Color Editor".to_string(),
        min_size: SIZE,
        max_size: Some(SIZE),
        ..Default::default()
    })
    ...
    window.render_loop(move |mut frame_input| {
        ...
        let (_, aftik_color_data) = colors.get_index(selected_index).unwrap();
        draw_examples(
    ...
    egui::SidePanel::right("side")
        ...
        .show(ctx, |ui| {
            ...
            if ui.button("Add").clicked() && !new_color_name.is_empty() {
                init_new_color(SpeciesColorId(take(new_color_name)), selected_index, colors);
            }
            ...
            egui::ComboBox::from_id_salt("color_id").show_index(
                ...
                colors.len(),
                |index| {
                    let (SpeciesColorId(name), _) = colors.get_index(index).unwrap();
                    name.to_owned()
                },
            );
            ...
            let (_, color_data) = colors.get_index_mut(*selected_index).unwrap();

            ui.label("Primary color:");
            color_picker(ui, &mut color_data.color_data.primary_color);

            ui.separator();

            ui.label("Secondary color:");
            color_picker(ui, &mut color_data.color_data.secondary_color);
...
fn init_new_color(new_id: SpeciesColorId, selected_index: &mut usize, aftik_colors: &mut ColorMap) {
